...
class Wypozyczalnia implements Serializable {
    ...
    public void setXMLToDB(Login login) throws SQLException{
        stmt = con.prepareStatement("SELECT * FROM Login", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
        rs = stmt.executeQuery();
        ...
        rs.moveToInsertRow();
        rs.updateInt("idL", id);
        rs.updateString("dateL", login.getLoginDate());
        rs.updateString("nameL", login.getName());
        rs.insertRow();
    ...
    public void setDBToXML() throws SQLException, JAXBException{
        ArrayList<Login> logins = new ArrayList<Login>();
        stmt = con.prepareStatement("SELECT * FROM Login", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        rs = stmt.executeQuery();
        ...
        while(rs.getRow()!=0){
            date = rs.getString("dateL");
            name = rs.getString("nameL");
            login = new Login();
            login.setName(name);
        ...
        loginss.setLoginList(logins);
        jaxbMarshaller.marshal(loginss, file);
        stmt.close();
        rs.close();
    }
